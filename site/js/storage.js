...
export function createDefaultState(){return{version:1,theme:"system",language:"ca",errorScores:{},questionStats:{},history:[]};}
export function loadState(storage=window.localStorage){const raw=storage.getItem(STORAGE_KEY);if(!raw)return createDefaultState();try{const p=JSON.parse(raw);if(p.version!==1)return createDefaultState();const state={...createDefaultState(),...p};state.language=state.language==="es"?"es":"ca";return state;}catch{return createDefaultState();}}
...
export function resetProgress(storage=window.localStorage){const c=loadState(storage);const r=createDefaultState();r.theme=c.theme;r.language=c.language;saveState(r,storage);}
