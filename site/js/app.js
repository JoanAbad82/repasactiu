...
const els={
 home:document.querySelector('#home-screen'),setup:document.querySelector('#setup-screen'),quiz:document.querySelector('#quiz-screen'),results:document.querySelector('#results-screen'),review:document.querySelector('#review-screen'),study:document.querySelector('#study-cards-screen'),live:document.querySelector('#live-region'),
...
function openSetup(sel,review=false){if(!askLeave())return;exitStudyMode();selection=sel;setup={mode:'study',count:10,penaltyEnabled:false,review};renderSetupScreen();}
function bindSetup(){
...
function bindGlobal(){
 document.querySelector('#home-link').addEventListener('click',goHome);els.syllabus.addEventListener('click',goHome);els.reviewErrors.addEventListener('click',()=>openSetup('all',true));els.studyLink.addEventListener('click',openStudyCards);
...
async function init(){try{syncTheme();syncLanguageChrome();[course,studyBank]=await Promise.all([loadCourse(),loadStudyCardsBank()]);const corrections=await loadContentCorrections();banks=await Promise.all(course.blocks.map(async meta=>{
...
}));bindGlobal();els.studyLink.disabled=false;els.studyLink.setAttribute('aria-disabled','false');renderHome();}catch(error){syncLanguageChrome();els.home.innerHTML=`<div class="error-message"><strong>${t().loadError}</strong><p>${t().retry}</p></div>`;console.error(error);}}
init();
