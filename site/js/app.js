...
function syncLanguageChrome(){
 ...
 els.syllabus.textContent=t().syllabus; els.reviewErrors.textContent=t().review; els.studyLink.textContent=t().study; els.dictionaryLink.textContent=t().dictionary; els.theme.textContent=t().theme; els.theme.setAttribute('aria-label',t().themeAria); els.nav.setAttribute('aria-label',t().nav); els.languageGroup.setAttribute('aria-label',t().language);
}
...
function exitDictionaryMode(){
 if(dictionaryView){dictionaryView.destroy();dictionaryView=null;}
}
...
function bindGlobal(){
 document.querySelector('#home-link').addEventListener('click',goHome);els.syllabus.addEventListener('click',goHome);els.reviewErrors.addEventListener('click',()=>openSetup('all',true));els.studyLink.addEventListener('click',openStudyCards);els.dictionaryLink.addEventListener('click',openConceptDictionary);
...
async function init(){try{syncTheme();syncLanguageChrome();[course,studyBank,dictionaryBank]=await Promise.all([loadCourse(),loadStudyCardsBank(),loadConceptDictionary()]);const corrections=await loadContentCorrections();banks=await Promise.all(course.blocks.map(async meta=>{
...
}));bindGlobal();els.studyLink.disabled=false;els.studyLink.setAttribute('aria-disabled','false');els.dictionaryLink.disabled=false;els.dictionaryLink.setAttribute('aria-disabled','false');renderHome();}catch(error){syncLanguageChrome();els.home.innerHTML=`<div class="error-message"><strong>${t().loadError}</strong><p>${t().retry}</p></div>`;console.error(error);}}
init();
