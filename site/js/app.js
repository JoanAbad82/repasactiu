import {loadCourse,loadBlockBundle} from './catalog.js';
...
function bindSetup(){
 ...
 els.setup.onchange=e=>{if(e.target.name==='mode'){setup.mode=e.target.value;const field=els.setup.querySelector('#penalty-field');if(field)field.hidden=setup.mode!=='exam';}if(e.target.name==='count')setup.count=e.target.value==='all'?'all':Number(e.target.value);if(e.target.name==='penalty')setup.penaltyEnabled=e.target.value==='on';};
...
function nextQuestion(){if(session.mode!=='exam'&&!session.revealed)return;if(session.index<session.questions.length-1){session.index++;session.revealed=false;renderCurrent();return;}finishQuiz();}
...
async function init(){try{syncTheme();syncLanguageChrome();course=await loadCourse();banks=await Promise.all(course.blocks.map(async meta=>{const bank=await loadBlockBundle(meta.file,meta.extraFile,fetch,meta.translationFile,meta.memoryAidFile,meta.additionalFiles||[],meta.additionalTranslationFiles||[],meta.additionalMemoryAidFiles||[]);if(bank.blockId!==meta.id)throw new Error('El banc no correspon al bloc declarat al curs.');return {...bank,blockTitleEs:meta.titleEs||bank.blockTitleEs,unitId:meta.unitId,unitTitle:meta.unitTitle,unitTitleEs:meta.unitTitleEs,blockNumber:meta.blockNumber};}));bindGlobal();renderHome();}catch(error){syncLanguageChrome();els.home.innerHTML=`<div class="error-message"><strong>${t().loadError}</strong><p>${t().retry}</p></div>`;console.error(error);}}
init();
