...
function syncLanguageChrome(){
 ...
 els.syllabus.textContent=t().syllabus; els.reviewErrors.textContent=t().review; els.studyLink.textContent=t().study; els.dictionaryLink.textContent=t().dictionary; els.correspondenceLink.textContent=t().correspondence; els.theme.textContent=t().theme; els.theme.setAttribute('aria-label',t().themeAria); els.nav.setAttribute('aria-label',t().nav); els.languageGroup.setAttribute('aria-label',t().language);
}
...
}));bindGlobal();els.studyLink.disabled=false;els.studyLink.setAttribute('aria-disabled','false');els.dictionaryLink.disabled=false;els.dictionaryLink.setAttribute('aria-disabled','false');els.correspondenceLink.disabled=false;els.correspondenceLink.setAttribute('aria-disabled','false');renderHome();}catch(error){syncLanguageChrome();els.home.innerHTML=`<div class="error-message"><strong>${t().loadError}</strong><p>${t().retry}</p></div>`;console.error(error);}}
init();
