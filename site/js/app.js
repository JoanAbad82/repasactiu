...
import {loadMemoryGameBank,createMemoryGame} from './memory-game.js';

const els={
 home:document.querySelector('#home-screen'),setup:document.querySelector('#setup-screen'),quiz:document.querySelector('#quiz-screen'),results:document.querySelector('#results-screen'),review:document.querySelector('#review-screen'),memory:document.querySelector('#memory-game-screen'),live:document.querySelector('#live-region'),
 syllabus:document.querySelector('#syllabus-link'),reviewErrors:document.querySelector('#review-errors-link'),memoryLink:document.querySelector('#memory-game-link'),theme:document.querySelector('#theme-toggle'),langCa:document.querySelector('#language-ca'),langEs:document.querySelector('#language-es'),nav:document.querySelector('.site-header nav'),languageGroup:document.querySelector('.language-switcher')
};
const chromeCopy={
...
function syncLanguageChrome(){
 ...
 els.syllabus.textContent=t().syllabus; els.reviewErrors.textContent=t().review; els.memoryLink.textContent='Tarjetas de memoria'; els.theme.textContent=t().theme; els.theme.setAttribute('aria-label',t().themeAria); els.nav.setAttribute('aria-label',t().nav); els.languageGroup.setAttribute('aria-label',t().language);
}
...
function rerenderCurrentScreen(){syncLanguageChrome();if(currentScreen==='home-screen')renderHome();else if(currentScreen==='setup-screen'){if(els.setup.querySelector('.empty-state'))renderEmptyReview();else renderSetupScreen();}else if(currentScreen==='quiz-screen'&&session)renderCurrent();else if(currentScreen==='results-screen'&&lastResult)renderResultsScreen();else if(currentScreen==='review-screen'&&session)renderReviewScreen();else if(currentScreen==='memory-game-screen'){syncLanguageChrome();}}
...
function bindGlobal(){
 document.querySelector('#home-link').addEventListener('click',goHome);els.syllabus.addEventListener('click',goHome);els.reviewErrors.addEventListener('click',()=>openSetup('all',true));els.memoryLink.addEventListener('click',openMemoryGame);
...
async function init(){try{syncTheme();syncLanguageChrome();[course,memoryBank]=await Promise.all([loadCourse(),loadMemoryGameBank()]);const corrections=await loadContentCorrections();banks=await Promise.all(course.blocks.map(async meta=>{
