import {composeHardQuestion} from './hard-distractors.js';
...
export function buildHardQuiz(questions,hardById,requestedCount,rng=Math.random){const prepared=questions.map(q=>{const record=hardById?.[q.id];if(!record)throw new Error(`${q.id}: hard distractor record absent`);return composeHardQuestion(q,record);});return buildQuiz(prepared,requestedCount,rng);}
