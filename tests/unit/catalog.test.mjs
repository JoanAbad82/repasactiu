...
const okFetch = async (url) => ({
  ok: true,
  json: async () => url.endsWith("course.json")
    ? {id:"course",title:"Curs",blocks:[]}
    : {blockId:"bloc-1",blockTitle:"Bloc 1",questions:url.includes("expansion")?[{id:"q3"}]:url.includes("extra")?[{id:"q2"}]:[{id:"q1"}]}
});
...
test("loadBlockBundle fusiona base, extra i fitxers addicionals en ordre", async()=> {
  const bank=await loadBlockBundle("data/bloc_1.json","data/bloc_1_extra.json",okFetch,null,null,["data/bloc_1_expansion.json"]);
  assert.deepEqual(bank.questions.map(q=>q.id),["q1","q2","q3"]);
});
...
test("loadBlockBundle rebutja un fitxer d'expansió d’un altre bloc", async()=> {
  const mismatch=async url=>({ok:true,json:async()=>url.includes("expansion")?{blockId:"bloc-2",questions:[{id:"q3"}]}:url.includes("extra")?{blockId:"bloc-1",questions:[{id:"q2"}]}:{blockId:"bloc-1",questions:[{id:"q1"}]}});
  await assert.rejects(()=>loadBlockBundle("data/bloc_1.json","data/bloc_1_extra.json",mismatch,null,null,["data/bloc_1_expansion.json"]),/no correspon al bloc principal/);
});
